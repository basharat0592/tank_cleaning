...
# Set page config
st.set_page_config(
    page_title="Tank Cleaning Graph Analyzer (Staging)",
    page_icon="🛢️",
    layout="wide"
)
...
# PostgreSQL settings
POSTGRES_CONN = {
    "host": os.getenv("POSTGRES_HOST", "postgres_age_pgvector"),
    "port": "5432",
    "dbname": "postgres",
    "user": "postgres",
...
def store_document(content, source):
    try:
        if not check_and_create_documents_table():
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO documents (content, embedding, source, created_at)
            VALUES (%s, %s, %s, %s)
            RETURNING id;
        """, (content, embedding_str, source, datetime.now()))
...
def semantic_search(query, limit=3):
    try:
        ...
        return [{"id": r[0], "content": r[1], "source": r[2], "created_at": r[3].isoformat() if r[3] else None} for r in results]
    except Exception as e:
...
def generate_sample_csv(table_name):
    if table_name == "Customers":
        ...
        df = pd.DataFrame([
            {
                "Content": "Container HOYU 000001-3 had a valve leakage issue on May 3, reported by Driver John.",
        ...
        if table_name == "Documents":
            if file_type == "csv":
                ...
                for _, row in df.iterrows():
                    ...
                    if not store_document(content, source):
...
def process_user_query(user_question):
    ...
    if "contents of documents" in user_question.lower():
        try:
            ...
            conn.autocommit = True
            cur = conn.cursor()
            cur.execute("SELECT id, content, source, created_at FROM public.documents ORDER BY id ASC")
            results = cur.fetchall()
            doc_results = [{"id": r[0], "content": r[1], "source": r[2], "created_at": r[3].isoformat() if r[3] else None} for r in results]
            cur.close()
            conn.close()
    ...
    while attempts < MAX_ATTEMPTS:
        ...
        doc_context = "\n".join([f"Document {d['id']} (Source: {d['source']}): {d['content']}" for d in doc_results]) if doc_results else "No relevant documents found."
...
# Footer
st.markdown("---")
st.caption("Tank Cleaning Graph Analyzer (Staging) | Powered by Apache AGE, PGVector, and Azure OpenAI")